class Heap:
    ...
    def extract_max(self):
        if self.size == 0:
            return None
        max_val = self.heap[self.size//2+1]
        max_ind = self.size//2+1
        for i in range(1+self.size//2, self.size+1):
            if self.heap[i] > max_val:
                max_val = self.heap[i]
                max_ind = i
        self.heap[max_ind], self.heap[self.size] = self.heap[self.size], self.heap[max_ind]
        self.heap.pop()
        self.size -= 1
        self.heapify_up()
        return max_val[1] 
