...
int main(){
    struct term *terms;
    int nterms;
    read_in_terms(&terms, &nterms, "L152.txt");

    int low = lowest_match(terms, nterms, "Toronto");
    int high = highest_match(terms, nterms, "Toronto");
    printf("%d %d", low, high);
    struct term *answer;
    int n_answer;
    autocomplete(&answer, &n_answer, terms, nterms, "Toronto");
    printf("%d", n_answer);
}